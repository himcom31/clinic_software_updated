...
const globalStyles = `
  ...
  .preview-modal-overlay {
    ...
    padding: 16px; background: rgba(10,15,30,0.82); backdrop-filter: blur(6px);
  }
  .preview-modal {
  ...
  .preview-pdf-container {
    background: #f0f4f8; padding: 16px; flex: 1;
  ...
  .preview-btn {
    border: none; border-radius: 8px; padding: 13px 10px;
    font-weight: 800; font-size: 11px; text-transform: uppercase;
    letter-spacing: 0.8px; cursor: pointer;
    display: flex; flex-direction: column; align-items: center; justify-content: center;
    gap: 6px; transition: all 0.18s; line-height: 1.2;
  }
  .preview-btn-wa { background: #25D366; color: #fff; }
  .preview-btn-wa:hover { background: #1ebe5d; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(37,211,102,0.35); }
  ...
  .preview-btn-print:hover { background: #1e293b; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(51,65,85,0.3); }
  .preview-btn-exit { background: #16a34a; color: #fff; }
  .preview-btn-exit:hover { background: #15803d; transform: translateY(-1px); box-shadow: 0 4px 14px rgba(22,163,74,0.3); }
...
const PreviewModal = ({ isOpen, onClose, pdfDoc, patient, onPrint, onSaveExit, navigate, slug }) => {
    ...
    const handleWhatsApp = async () => {
        ...
        try {
            const configRes = await axios.get(`${API_BAS}/api/notifications/${slug}`);
            const config = configRes.data.data || {};
            if (!config.waEnabled) { setWaStatus('error'); setWaError('WhatsApp notifications are not enabled for this clinic. Please configure them in Settings.'); setWaSending(false); return; }
            if (!config.waPhoneId || !config.waToken) { setWaStatus('error'); setWaError('WhatsApp Phone ID or Token is missing in notification settings.'); setWaSending(false); return; }
            const formData = new FormData();
            const pdfBlob = pdfDoc.output('blob');
            formData.append('file', pdfBlob, `prescription_${patient.name || 'patient'}.pdf`);
            formData.append('type', 'application/pdf');
            formData.append('messaging_product', 'whatsapp');
            const uploadRes = await fetch(`https://graph.facebook.com/v19.0/${config.waPhoneId}/media`, { method: 'POST', headers: { Authorization: `Bearer ${config.waToken}` }, body: formData });
            const uploadData = await uploadRes.json();
            if (!uploadData.id) throw new Error(uploadData.error?.message || 'PDF upload to WhatsApp failed.');
            const mediaId = uploadData.id;
            let mobile = String(patient.mobile).replace(/\D/g, '');
            if (!mobile.startsWith('91') && mobile.length === 10) mobile = '91' + mobile;
            const msgPayload = { messaging_product: 'whatsapp', to: mobile, type: 'document', document: { id: mediaId, filename: `Prescription_${patient.name || 'Patient'}.pdf`, caption: config.waTemplate ? config.waTemplate.replace('{{name}}', patient.name || 'Patient') : `Dear ${patient.name || 'Patient'}, please find your prescription attached. Get well soon!` } };
            const sendRes = await fetch(`https://graph.facebook.com/v19.0/${config.waPhoneId}/messages`, { method: 'POST', headers: { Authorization: `Bearer ${config.waToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(msgPayload) });
            const sendData = await sendRes.json();
            if (sendData.messages?.[0]?.id) { setWaStatus('success'); } else { throw new Error(sendData.error?.message || 'Failed to send WhatsApp message.'); }
        } catch (err) { setWaStatus('error'); setWaError(err.message || 'WhatsApp send failed.'); }
            ...
            <div className="preview-modal">
                <div className="preview-modal-header">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ background: '#1976D2', borderRadius: 6, padding: '6px 8px', color: '#fff', display: 'flex' }}><Eye size={15} /></div>
                        <div>
                            <div style={{ fontSize: 13, fontWeight: 800, color: '#0f172a', letterSpacing: '-0.2px' }}>Prescription Ready</div>
                            <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600 }}>{patient?.name} · {patient?.mobile}</div>
                        </div>
                    </div>
                    <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: 4, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        onMouseEnter={e => e.currentTarget.style.color = '#ef4444'} onMouseLeave={e => e.currentTarget.style.color = '#94a3b8'}>
                        <X size={18} />
                    </button>
                </div>
                <div className="preview-pdf-container">
                    {pdfBlobUrl ? (
                        <iframe src={`${pdfBlobUrl}#toolbar=0&navpanes=0&scrollbar=0`} className="preview-pdf-frame" style={{ height: 380 }} title="Prescription Preview" />
                    ) : (
                    ...
                    {waStatus === 'success' && <div className="wa-status-badge success"><CheckCircle size={12} />Sent successfully on WhatsApp!</div>}
                ...
                <div className="preview-action-bar">
                    ...
                    <button className="preview-btn preview-btn-exit" onClick={handleSaveExit} disabled={waSaveExiting}>
                        {waSaveExiting ? <Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> : <LogOut size={18} />}
...
const GeneratePrescription = () => {
        ...
        const fetchClinicProfile = async () => {
            try {
                const [clinicRes, doctorRes] = await Promise.all([
                    axios.get(`${API_BAS}/api/clinic/${slug}/clinicData`),
                    axios.get(`${API_BAS}/api/doctors/${slug}/profile`), // adjust endpoint to match your actual doctor API
                ]);
                setClinicProfile(clinicRes.data.data);
                setDoctorName(doctorRes.data.data?.name || doctorRes.data.data?.drName || '');
            } catch (err) {
    ...
    /* ── Build PDF ── */
    const buildPdfDoc = (design, patient, formStructure) => {
        ...
        const addContinuationPage = () => {
            ...
            doc.text(MARGIN_L, PAGE2_START_Y - 20);
        ...
        // ── Doctor Signature Block (after last content, before footer) ──
        curY = checkPageBreak(curY, 80);
        curY += 20; // breathing space after reports

        const sigBlockX = MARGIN_R - 160; // right-aligned block
        ...
        if (clinicProfile?.signature) {
            try {
                doc.addImage(
                    ...
                    clinicProfile.signature.includes('image/png') ? 'PNG' : 'JPEG',
                    sigBlockX,
                    curY,
                    120,  // width
                    40,   // height
        ...
        doc.text(
            doctorName || design?.drName || 'Doctor Name',
            sigBlockX + 80,
            curY + 10,
            { align: 'center' }
        );
        ...
        doc.text(
            `Reg. No: ${clinicProfile?.regNo || design?.regNo || '—'}`,
            sigBlockX + 80,
            curY + 4,
            { align: 'center' }
        );
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const doc = buildPdfDoc(design, patient, formStructure);
